...
alpaka::enqueue(queue,
		alpaka::createTaskKernel<Acc1>(workDivTriplets,
					       kernelFastFit<3>(),
					       tuples_d, tupleMultiplicity_d, 3, hv, alpaka::getPtrNative(hitsGPU_), alpaka::getPtrNative(hits_geGPU_), alpaka::getPtrNative(fast_fit_resultsGPU_), offset));
...
// quads
 alpaka::enqueue(queue,
		 alpaka::createTaskKernel<Acc1>(workDivQuadsPenta,
						kernelFastFit<4>(),
						tuples_d, tupleMultiplicity_d, 4, hv, alpaka::getPtrNative(hitsGPU_), alpaka::getPtrNative(hits_geGPU_), alpaka::getPtrNative(fast_fit_resultsGPU_), offset));
 ...
 if (fit5as4_) {
   ...
   alpaka::enqueue(queue,
		      alpaka::createTaskKernel<Acc1>(workDivQuadsPenta,
						     kernelFastFit<4>(),
						     tuples_d, tupleMultiplicity_d, 5, hv, alpaka::getPtrNative(hitsGPU_), alpaka::getPtrNative(hits_geGPU_), alpaka::getPtrNative(fast_fit_resultsGPU_), offset));
