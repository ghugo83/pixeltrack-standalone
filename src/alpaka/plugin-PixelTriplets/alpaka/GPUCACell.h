...
public:
  ...
  template <typename T_Acc>
    ALPAKA_FN_ACC ALPAKA_FN_INLINE __attribute__((always_inline)) int addOuterNeighbor(const T_Acc &acc, CellNeighbors::value_t t, CellNeighborsVector& cellNeighbors) {
    // use smart cache
  ...
  ALPAKA_FN_ACC ALPAKA_FN_INLINE __attribute__((always_inline)) CellTracks const& tracks() const { return *theTracks; }
  ALPAKA_FN_ACC ALPAKA_FN_INLINE __attribute__((always_inline)) CellNeighbors& outerNeighbors() { return *theOuterNeighbors; }
  ALPAKA_FN_ACC ALPAKA_FN_INLINE __attribute__((always_inline)) CellNeighbors const& outerNeighbors() const { return *theOuterNeighbors; }
  ALPAKA_FN_ACC ALPAKA_FN_INLINE __attribute__((always_inline)) float get_inner_x(Hits const& hh) const { return hh.xGlobal(theInnerHitId); }
  ALPAKA_FN_ACC ALPAKA_FN_INLINE __attribute__((always_inline)) float get_outer_x(Hits const& hh) const { return hh.xGlobal(theOuterHitId); }
  ALPAKA_FN_ACC ALPAKA_FN_INLINE __attribute__((always_inline)) float get_inner_y(Hits const& hh) const { return hh.yGlobal(theInnerHitId); }
  ALPAKA_FN_ACC ALPAKA_FN_INLINE __attribute__((always_inline)) float get_outer_y(Hits const& hh) const { return hh.yGlobal(theOuterHitId); }
  ALPAKA_FN_ACC ALPAKA_FN_INLINE __attribute__((always_inline)) float get_inner_z(Hits const& hh) const { return theInnerZ; }
  // { return hh.zGlobal(theInnerHitId); } // { return theInnerZ; }
  ALPAKA_FN_ACC ALPAKA_FN_INLINE __attribute__((always_inline)) float get_outer_z(Hits const& hh) const { return hh.zGlobal(theOuterHitId); }
  ALPAKA_FN_ACC ALPAKA_FN_INLINE __attribute__((always_inline)) float get_inner_r(Hits const& hh) const { return theInnerR; }
  // { return hh.rGlobal(theInnerHitId); } // { return theInnerR; }
  ALPAKA_FN_ACC ALPAKA_FN_INLINE __attribute__((always_inline)) float get_outer_r(Hits const& hh) const { return hh.rGlobal(theOuterHitId); }

  ALPAKA_FN_ACC ALPAKA_FN_INLINE __attribute__((always_inline)) auto get_inner_iphi(Hits const& hh) const { return hh.iphi(theInnerHitId); }
  ALPAKA_FN_ACC ALPAKA_FN_INLINE __attribute__((always_inline)) auto get_outer_iphi(Hits const& hh) const { return hh.iphi(theOuterHitId); }

  ALPAKA_FN_ACC ALPAKA_FN_INLINE __attribute__((always_inline)) float get_inner_detIndex(Hits const& hh) const { return hh.detectorIndex(theInnerHitId); }
  ALPAKA_FN_ACC ALPAKA_FN_INLINE __attribute__((always_inline)) float get_outer_detIndex(Hits const& hh) const { return hh.detectorIndex(theOuterHitId); }
  ...
  ALPAKA_FN_ACC ALPAKA_FN_INLINE __attribute__((always_inline)) static bool dcaCutH(float x1,
                                                 float y1,
                                                 float x2,
                                                 float y2,
                                                 float x3,
  ...
  ALPAKA_FN_ACC ALPAKA_FN_INLINE void find_ntuplets(const T_Acc &acc, 
    ...
    for (int j = 0; j < outerNeighbors().size(); ++j) {
      ...
      last = false;
      cells[otherCell].find_ntuplets(acc, hh, cells, cellTracks, foundNtuplets, apc, quality, tmpNtuplet, minHitsPerNtuplet, startAt0);
    }
