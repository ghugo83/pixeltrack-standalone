...
namespace gpuPixelDoublets {
  ...
  ALPAKA_FN_ACC ALPAKA_FN_INLINE __attribute__((always_inline)) void doubletsFromHisto(const T_Acc& acc,
    ...
    for (uint32_t j = firstElementIdxY; j < ntot; ++j) {
      if (!cms::alpakatools::get_next_element_1D_index_stride(j, firstElementIdxY, endElementIdxY, gridDimensionY, ntot))
	break;
	...
	for (uint32_t pIndex = firstElementIdxX; pIndex < maxpIndex; ++pIndex) {
	  if (!cms::alpakatools::get_next_element_1D_index_stride(pIndex, firstElementIdxX, endElementIdxX, blockDimensionX, maxpIndex))
	    break;
