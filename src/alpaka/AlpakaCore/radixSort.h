...
ALPAKA_FN_ACC ALPAKA_FN_INLINE void dummyReorder(const T_Acc& acc, T const* a, uint16_t* ind, uint16_t* ind2, uint32_t size) {}

template <typename T_Acc, typename T>
ALPAKA_FN_ACC ALPAKA_FN_INLINE void reorderSigned(const T_Acc& acc, T const* a, uint16_t* ind, uint16_t* ind2, uint32_t size) {
  //move negative first...
...
cms::alpakatools::for_each_element_1D_block_stride(acc, size, firstNeg, [&](uint32_t i) {
    ind2[i - firstNeg] = ind[i];
});
  alpaka::syncBlockThreads(acc);

cms::alpakatools::for_each_element_1D_block_stride(acc, firstNeg, [&](uint32_t i) {
    ind2[i + size - firstNeg] = ind[i];
});
  alpaka::syncBlockThreads(acc);
...
cms::alpakatools::for_each_element_1D_block_stride(acc, size, [&](uint32_t i) {
 ...
 template <typename T_Acc, typename T>
ALPAKA_FN_ACC ALPAKA_FN_INLINE void reorderFloat(const T_Acc& acc, T const* a, uint16_t* ind, uint16_t* ind2, uint32_t size) {
  //move negative first...
...
template <typename T_Acc, 
  typename T,  // shall be interger
          ...
          typename RF>
ALPAKA_FN_ACC ALPAKA_FN_INLINE __attribute__((always_inline)) void radixSortImpl(const T_Acc& acc, 
